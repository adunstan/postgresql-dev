...
typedef struct CopyStateData
{
	...
	/* Datum representation of fields, used by COPY FROM. */
	Datum	   *values;
	bool	   *nulls;
...
CopyFrom(CopyState cstate)
{
	...
	cstate->values = (Datum *) palloc(tupDesc->natts * sizeof(Datum));
	cstate->nulls = (bool *) palloc(tupDesc->natts * sizeof(bool));
	...
	while (!done)
	{
		...
		tuple = NextCopyFrom(cstate);

		/* Triggers and stuff need to be invoked in query context. */
		MemoryContextSwitchTo(oldcontext);

		/* We break here to ensure current memory context has been restored. */
...
NextCopyFrom(CopyState cstate)
{
	...
	/* XXX: Indentation is not adjusted to keep the patch small. */
		...
		MemSet(cstate->values, 0, num_phys_attrs * sizeof(Datum));
		MemSet(cstate->nulls, true, num_phys_attrs * sizeof(bool));
		...
		if (!cstate->binary)
		{
			...
			foreach(cur, cstate->attnumlist)
			{
				...
				cstate->cur_attval = string;
				cstate->values[m] = InputFunctionCall(&in_functions[m],
													  string,
													  typioparams[m],
													  attr[m]->atttypmod);
				if (string != NULL)
					cstate->nulls[m] = false;
				cstate->cur_attname = NULL;
				cstate->cur_attval = NULL;
			}
			}
		...
		else
		{
			...
			foreach(cur, cstate->attnumlist)
			{
				...
				cstate->cur_attname = NameStr(attr[m]->attname);
				i++;
				cstate->values[m] = CopyReadBinaryAttribute(cstate,
															i,
															&in_functions[m],
															typioparams[m],
															attr[m]->atttypmod,
		...
		{
			cstate->values[defmap[i]] = ExecEvalExpr(defexprs[i], econtext,
													 &cstate->nulls[defmap[i]],
													 NULL);
		}
	/* XXX: End of only-indentation changes. */

	tuple = heap_form_tuple(tupDesc, cstate->values, cstate->nulls);
	if (tupleOid != InvalidOid)
